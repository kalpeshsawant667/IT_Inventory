...
export default function AssetForm() {
  ...
  const [form, setForm] = useState({
    asset_tag: '', name: '', category_id: '', model: '', manufacturer: '', serial_number: '',
    specifications_json: '', purchase_date: '', warranty_expiry: '', purchase_cost: '',
    vendor_id: '', status: 'AVAILABLE', location_id: '', assigned_to_user_id: '', notes: ''
  })
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      api.get(`/assets/${id}`).then((res) => {
        const a = res.data
        setForm({
          asset_tag: a.asset_tag || '', name: a.name || '', category_id: a.category_id || '',
          model: a.model || '', manufacturer: a.manufacturer || '', serial_number: a.serial_number || '',
          specifications_json: a.specifications_json || '', purchase_date: a.purchase_date || '',
          warranty_expiry: a.warranty_expiry || '', purchase_cost: a.purchase_cost || '',
          vendor_id: a.vendor_id || '', status: a.status || 'AVAILABLE', location_id: a.location_id || '',
          assigned_to_user_id: a.assigned_to_user_id || '', notes: a.notes || ''
        })
      })
    }
  }, [id])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) await api.put(`/assets/${id}`, payload)
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div><label className="block text-sm font-medium text-gray-700">Asset Tag *</label>
            <input name="asset_tag" required value={form.asset_tag} onChange={handleChange} className="mt-1 input-field" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Name *</label>
            <input name="name" required value={form.name} onChange={handleChange} className="mt-1 input-field" /></div>
          ...
          <div><label className="block text-sm font-medium text-gray-700">Model</label>
            <input name="model" value={form.model} onChange={handleChange} className="mt-1 input-field" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Manufacturer</label>
            <input name="manufacturer" value={form.manufacturer} onChange={handleChange} className="mt-1 input-field" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Serial Number</label>
            <input name="serial_number" value={form.serial_number} onChange={handleChange} className="mt-1 input-field" /></div>
          ...
          <div><label className="block text-sm font-medium text-gray-700">Assigned To</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700">Purchase Date</label>
            <input type="date" name="purchase_date" value={form.purchase_date} onChange={handleChange} className="mt-1 input-field" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Warranty Expiry</label>
            <input type="date" name="warranty_expiry" value={form.warranty_expiry} onChange={handleChange} className="mt-1 input-field" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Purchase Cost</label>
            <input type="number" step="0.01" name="purchase_cost" value={form.purchase_cost} onChange={handleChange} className="mt-1 input-field" /></div>
        </div>
        <div><label className="block text-sm font-medium text-gray-700">Notes</label>
          <textarea name="notes" rows={3} value={form.notes} onChange={handleChange} className="mt-1 input-field" /></div>
